...
from numpy import exp, dot, zeros, random, dtype, float32 as REAL,\
    ...
    empty, sum as np_sum, ones, logaddexp, log, outer, mean, abs, ones_like
...
try:
    raise ImportError   # To force use of rel_weights version
    ...
    def train_batch_sg(model, sentences, alpha, work=None, compute_loss=False, relational_weighting=False, ave_weight=None, trans_factor=None):
        ...
        relational_weighting : bool, optional
            Whether sentences parameter is actually a tuple, where the first entry is the
            regular sentences parameter and the second is the weights between each word
            in the sentence.
        ...
        for i, sentence in enumerate(sentences):
            if relational_weighting:
                sentence, rel_weights = sentence
                if(len(sentence)-1 != len(rel_weights)):
                    print(sentence)
                    print(rel_weights)
            ...
            for pos, word in enumerate(word_vocabs):
                ...
                for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                    ...
                    if pos2 != pos:
                        if relational_weighting:
                            rel_sign = reduce(mul, rel_weights[pos:pos2], 1) 
                            if rel_sign > 0:
                                rel_sign = 1
                            else:
                                rel_sign = -1
                            nodes = len(rel_weights[pos:pos2])
                            if nodes == 0: continue
                            rel_weight = rel_sign * mean(abs(rel_weights[pos:pos2])) * (trans_factor ** (nodes-1))
                            if rel_weight == 0: continue
                        else:
                            rel_weight = None
                        train_sg_pair(
                            model, model.wv.index2word[word.index], word2.index, alpha, compute_loss=compute_loss,
                            relational_weighting=relational_weighting, rel_weight = rel_weight, ave_weight = ave_weight
                        )
...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
                  context_vectors=None, context_locks=None, compute_loss=False, is_ft=False, relational_weighting=False,
                  rel_weight=None, ave_weight=None):
    ...
    if model.hs:
        ...
        if relational_weighting and rel_weight != 0:
            prod_term[0] = prod_term[0] / rel_weight
            prod_term[1:] = prod_term[1:] / ave_weight
    ...
    if model.negative:
        ...
        if relational_weighting:
            gb = (ones_like(model.neg_labels) - fb) * alpha  # vector of error gradients multiplied by the learning rate
        else:
        ...
        if compute_loss:
            ...
            #    prod_term[1:] = prod_term[1:] * ave_weight
            if relational_weighting:
                model.running_training_loss -= sum(log(expit(prod_term[1:])))  # for the sampled words
...
class Word2Vec(BaseWordEmbeddingsModel):
    ...
    def __init__(self, sentences=None, corpus_file=None, size=100, alpha=0.025, window=5, min_count=5,
                 ...
                 max_final_vocab=None, relational_weighting=False, ave_weight=None, trans_factor=None):
        """
        ...
        self.vocabulary = Word2VecVocab(
            ...
            null_word=null_word, max_final_vocab=max_final_vocab, ns_exponent=ns_exponent, relational_weighting=relational_weighting)
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        if self.sg:
            if FAST_VERSION < 0:
                tally += train_batch_sg(self, sentences, alpha, work, self.compute_loss, self.relational_weighting, ave_weight=self.ave_weight,
                                            trans_factor = self.trans_factor)
            else:
...
class Word2VecVocab(utils.SaveLoad):
    ...
    def __init__(
            ...
            max_final_vocab=None, ns_exponent=0.75, relational_weighting=False):
